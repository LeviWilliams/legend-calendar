...
## Custom `renderItem` performance

When using a custom `renderItem` with `Calendar.List`, avoid spreading `item.calendarProps` into your component:

```tsx
// ❌ Slower: forces Calendar.List to build a new data array every render,
// which gives LegendList a new `data` reference and re-diffs all items.
<Calendar.List
  renderItem={({ item }) => (
...
Instead, read the shared config directly from context inside your custom component using `useCalendarListConfig`:

```tsx
// ✅ Faster: data array is stable, LegendList only re-renders items
// whose calendarMonthId actually changed (recycle/scroll).
import { useCalendarListConfig } from "@lazerlen/legend-calendar";

function MyCalendar(props: { calendarMonthId: string }) {
...
<Calendar.List
  renderItem={({ item }) => <MyCalendar calendarMonthId={item.id} />}
/>
```

`useCalendarListConfig` returns `null` when used outside of a `Calendar.List`, so the pattern is safe to use in components that can appear in both contexts.

## Listening to the visible months

You can listen to which months are currently visible by hooking into the [`onViewableItemsChange`](https://legendapp.com/open-source/list/v3/api/#onviewableitemschanged) prop:
