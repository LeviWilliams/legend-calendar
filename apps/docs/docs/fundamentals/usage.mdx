...
## Auto-scroll to active date range

By default, `Calendar.List` automatically scrolls to show the active date range when the component mounts. This is especially useful when you have a date range set to a month far in the future or past.

The auto-scroll feature:
- Scrolls to the month containing the **start date** of the first active range
- Only happens on mount (uses `initialScrollIndex` internally)
- Takes precedence over `calendarInitialMonthId` when enabled
- Is enabled by default but can be disabled

**Example with auto-scroll enabled (default):**
      ...
      calendarActiveDateRanges={[
        {
          ...
          endId: toDateId(addMonths(futureDate, 1)),
        },
      ]}
      // Auto-scrolls to show the future date range on mount
    />
  );
};
```
...
If you want to disable this behavior and use `calendarInitialMonthId` instead, set `calendarInitialScrollToActiveRange` to `false`:
...
<Calendar.List
  ...
  // Will scroll to January 2024 instead of June 2025
/>
```
